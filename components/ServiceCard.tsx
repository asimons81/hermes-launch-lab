...
export function ServiceCard({ service, featured = false }: { service: Service; featured?: boolean }) {
  ...
  return <article className={`offer ${featured ? 'offer--featured' : ''}`}><p className="offer__label">{featured ? 'MOST HANDS-ON' : `${service.durationMin} MINUTES`}</p><h2 className="offer__title">{service.name}</h2><p className="offer__price">${service.price}{isCustom ? '+' : ''}</p><p>{service.description}</p><Link href={isCustom ? '/contact' : `/book?service=${service.slug}`} className="offer__link">{isCustom ? 'Apply via contact' : 'Choose this path'} <span>→</span></Link></article>
}
