...
export function ServiceCard({ service, featured = false }: { service: Service; featured?: boolean }) {
  return <article className={`offer ${featured ? 'offer--featured' : ''}`}><p className="offer__label">{featured ? 'MOST HANDS-ON' : `${service.durationMin} MINUTES`}</p><h2 className="offer__title">{service.name}</h2><p className="offer__price">${service.price}{service.slug === 'custom' ? '+' : ''}</p><p>{service.description}</p><Link href={`/book?service=${service.slug}`} className="offer__link">Choose this path <span>→</span></Link></article>
}
